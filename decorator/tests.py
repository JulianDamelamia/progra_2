...
def test_double_mocha():
    ...
    assert "Espresso Normal, Double Mocha" in PrettyPrintDecorator(bebida).get_description()

def test_soya_venti():
    bebida = build_beverage("HouseBlend", "Venti", ["Soy"])
    assert bebida.cost() == 0.89 + 0.20 + 0.15
    assert "Café de la Casa Venti, Soja" in PrettyPrintDecorator(bebida).get_description()

def test_triple_crema():
    bebida = build_beverage("HouseBlend", "Grande", ["Soy", "Whip", "Whip", "Whip"])
    assert "Café de la Casa Grande, Soja, Triple Crema" in PrettyPrintDecorator(bebida).get_description()
